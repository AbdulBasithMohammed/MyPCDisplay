...
class LcdComm(ABC):
    ...
    def DisplayRadialProgressBar(self, xc: int, yc: int, radius: int, bar_width: int,
        ...
        if with_text:
            ...
            # Callers right-justify the value to a minimum width (min_size), so
            # `text` arrives padded, e.g. "  8%". PIL's getbbox() counts those
            # leading spaces, so centring the padded string pushes the visible
            # glyphs right by half the padding - and by a different amount for
            # each digit count (12px at 8%, 6px at 41%, 0px at 100%), which
            # reads as the number wobbling around inside the ring.
            # Centre on the visible glyphs instead. The whole circle is
            # repainted from the background every frame, so dropping the
            # padding cannot cause ghosting here.
            visible = text.strip() or text
            left, top, right, bottom = ttfont.getbbox(visible)
            draw.text((radius - (left + right) / 2 + text_offset[0],
                       radius - (top + bottom) / 2 + text_offset[1]), visible,
