...
# Adds optional COLOR_THRESHOLDS support so a value can change color as it
# climbs (e.g. temperatures going blue -> amber -> red). Purely additive:
# themes without the key behave exactly as before.
#
#   COLOR_THRESHOLDS:
#     - ABOVE: 0
#       COLOR: 45, 164, 232
...
#       COLOR: 232, 150, 45
#
# Entries are evaluated in order; the last one whose ABOVE is <= value wins.
def _thresholded_color(theme_data, value, key, default):
    thresholds = theme_data.get(key, None)
    if not thresholds:
        return default
    try:
        numeric = float(value)
    except (TypeError, ValueError):
        # Custom sensors deliver an already-formatted string (e.g. "42°C"),
        # so pull the leading number out before comparing.
...
def display_themed_value(theme_data, value, min_size=0, unit=''):
    ...
    display.lcd.DisplayText(
        ...
        font_color=_thresholded_color(theme_data, value, "COLOR_THRESHOLDS",
                                      theme_data.get("FONT_COLOR", (0, 0, 0))),
...
def display_themed_progress_bar(theme_data, value):
    ...
    display.lcd.DisplayProgressBar(
        ...
        bar_color=_thresholded_color(theme_data, value, "BAR_COLOR_THRESHOLDS",
                                     theme_data.get("BAR_COLOR", (0, 0, 0))),
...
class CPU:
    ...
    def temperature(cls):
        ...
        # LOCAL MODIFICATION: also treat 0 as unavailable. A CPU at 0 C is not
        # physically plausible; it means the sensor backend was locked out (e.g.
        # the AMD Ryzen Master kernel driver holding the SMU) and returned a
        # placeholder. Showing a blank space is better than showing a wrong 0 C.
        if math.isnan(temperature) or temperature <= 0:
            ...
            if cpu_temp_text_data['SHOW'] or cpu_temp_radial_data['SHOW'] or cpu_temp_graph_data[
                ...
                logger.warning("CPU temperature unavailable (backend reported 0 or NaN) - hiding it")
...
# LOCAL MODIFICATION: remembers what each BITMAP element last drew, so an
# unchanged image is not pushed again. The panel is bandwidth-bound (~229 KB/s),
# and a large blit costs the best part of a second.
_LAST_BITMAP = {}


def display_themed_bitmap(key, theme_data, path):
    """Draw an image produced by a custom sensor.

    LOCAL MODIFICATION: upstream has no way for a sensor to drive an image -
    only text, bars and radials. The League screen composites its own layout
    with PIL and returns the file path, which this blits.
    """
    if not theme_data.get("SHOW", False) or not path:
        return
    ...
    try:
        ...
        return
    if _LAST_BITMAP.get(key) == stamp:
        return
    _LAST_BITMAP[key] = stamp
        ...
        for custom_stat in config.THEME_DATA['STATS']['CUSTOM']:
            if custom_stat != "INTERVAL":
                ...
                try:
                    ...
                    # LOCAL MODIFICATION: `continue`, not `return`. Upstream
                    # aborts the whole loop on one bad sensor, so a single
                    # failure silently blanks every custom element after it.
                    logger.error(
                ...
                # LOCAL MODIFICATION: image driven by the sensor's string value,
                # which is a file path. See display_themed_bitmap.
                theme_data = config.THEME_DATA['STATS']['CUSTOM'][custom_stat].get("BITMAP", None)
                if theme_data is not None and string_value:
                    display_themed_bitmap(str(custom_stat), theme_data, string_value)
